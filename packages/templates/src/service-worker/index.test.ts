...
describe("service-worker templates", () => {
  describe("getServiceWorkerTemplate", () => {
    ...
    it("should throw error for unknown type", () => {
      expect(() =>
        getServiceWorkerTemplate("unknown" as ServiceWorkerTemplateType),
      ).toThrow("Unknown service worker template type");
    });
  });
  ...
  describe("getAvailableTemplateTypes", () => {
    it("should return all available template types", () => {
      ...
      expect(types).toContain("next-ssr");
      expect(types).toContain("nuxt-ssr");
      expect(types).toContain("remix-ssr");
      expect(types).toContain("sveltekit-ssr");
  ...
  describe("template content validation", () => {
    it("should all templates use importScripts with Workbox CDN", () => {
      ...
      types.forEach((type) => {
        ...
        expect(template.content).toContain("importScripts");
        expect(template.content).toContain(
          "workbox-cdn/releases/7.4.0/workbox-sw.js",
        );
