...
function detectSymfonyVersion(projectRoot: string): string | null {
    try {
        ...
        const requiresDev = composer['require-dev'] as Record<string, string> | undefined

        const symfonyVersion = requires?.['symfony/framework-bundle'] ||
...
function hasSymfonyDependency(projectRoot: string): boolean {
    try {
        ...
        return !!(requires?.['symfony/framework-bundle'] ||
            requiresDev?.['symfony/framework-bundle'])
    } catch {
        return false
    }
...
function detectSPAMode(projectRoot: string): boolean {
    try {
        ...
        if (existsSync(join(projectRoot, 'webpack.config.js')) ||
...
function detectAPIPlatform(projectRoot: string): boolean {
    try {
        ...
        return !!(requires?.['api-platform/core'] ||
            requiresDev?.['api-platform/core'])
    } catch {
        return false
    }
}
...
export class SymfonyIntegration extends BaseBackendIntegration {
    ...
    detect(): BackendDetectionResult {
        ...
        const confidence = indicators.length >= 4 ? 'high' : indicators.length >= 2 ? 'medium' : 'low'
    ...
    generateServiceWorkerConfig(): ServiceWorkerConfig {
        const projectRoot = this.config.projectRoot
        const hasAPIPlatform = this.config.hasAPIPlatform ?? detectAPIPlatform(projectRoot)
    ...
    validateSetup(): Promise<{
        ...
        if (!existsSync(join(projectRoot, 'templates/offline.html.twig'))) {
            ...
            suggestions.push('Create templates/offline.html.twig for offline experience')
        }
