...
const createSymfonyProject = (): string => {
  const root = join(tmpdir(), `symfony-e2e-${Date.now()}-${Math.random().toString(36).slice(2)}`)
  ...
  // Create .env
  writeFileSync(join(root, '.env'), 'APP_NAME="Symfony Test App"\nAPP_ENV=dev\n')
...
describe('Symfony E2E - Full PWA Generation', () => {
  ...
  it('should use Symfony backend integration when detected', async () => {
  ...
  it('should generate Symfony-optimized service worker', async () => {
    const factory = getBackendFactory()
    const integration = factory.getIntegration('symfony', projectRoot)
    expect(integration).not.toBeNull()
    ...
    expect(staticPatterns.some((p) => p.includes('assets') || p.includes('build') || p.includes('bundles'))).toBe(true)
  })
