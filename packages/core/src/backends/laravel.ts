...
export class LaravelIntegration extends BaseBackendIntegration {
    ...
    /**
     * Detect if project is a Laravel application
     */
...
`
    ...
    validateSetup(): Promise<{
        ...
        if (!hasUrlRewrite) {
            ...
            suggestions.push('Ensure your web server properly routes requests to index.php')
        }

        // Check for PWAMiddleware
        if (!existsSync(join(projectRoot, 'app/Http/Middleware/PWAMiddleware.php'))) {
            suggestions.push('Consider registering PWAMiddleware for optimal PWA support')
        }
        ...
        if (!existsSync(join(projectRoot, 'resources/views/offline.blade.php'))) {
            ...
            suggestions.push('Create resources/views/offline.blade.php for offline experience')
        }
 ...
 */
export function createLaravelIntegration(projectRoot: string): LaravelIntegration {
