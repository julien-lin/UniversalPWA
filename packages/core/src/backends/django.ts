...
function detectDjangoVersion(projectRoot: string): string | null {
    ...
    try {
        ...
        if (existsSync(pyprojectPath)) {
            ...
            const match = content.match(/django\s*=\s*["']?[>=<~!]*(\d+)(?:\.(\d+))?(?:\.(\d+))?/i)
            if (match) {
                const major = match[1] ?? '0'
                const minor = match[2] ?? '0'
                const patch = match[3] ?? '0'
 ...
 */
function extractStaticFilesConfig(projectRoot: string): { staticUrl?: string; staticRoot?: string } {
    ...
    try {
        ...
        if (existsSync(settingsPath)) {
            ...
            // Extract STATIC_ROOT
            const staticRootMatch = content.match(/STATIC_ROOT\s*=\s*['"]([^'"]+)['"]/)
            if (staticRootMatch) {
...
export class DjangoIntegration extends BaseBackendIntegration {
    ...
    detect(): BackendDetectionResult {
        ...
        if (existsSync(join(projectRoot, 'settings.py')) || existsSync(join(projectRoot, 'settings'))) {
        ...
        if (hasDjangoDependency(projectRoot)) {
            indicators.push('Django dependency in requirements.txt or pyproject.toml')
            if (confidence === 'medium') {
                confidence = 'high'
            }
        }
    ...
    async validateSetup(): Promise<{
        ...
        if (existsSync(join(projectRoot, 'settings.py'))) {
            try {
                ...
                if (!/STATIC_ROOT/i.test(content)) {
                    suggestions.push('Consider setting STATIC_ROOT for production (used by collectstatic)')
                }
            } catch {
                // Ignore read errors
            }
        ...
        if (!this.config.isASGI) {
            suggestions.push('Consider using ASGI for better async support and WebSocket capabilities')
        }
