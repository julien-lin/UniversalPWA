...
describe("meta-injector", () => {
  ...
  describe("injectMetaTags", () => {
    ...
    it.each<Case>([
      ...
      {
        name: "mobile-web-app-capable (with apple-mobile-web-app-capable always present)",
      ...
      "should inject $name",
      ({ options, contains, notContains, injectedIncludes }) => {
        const html = baseHtml();
        ...
        expect(
          result.injected.some((i: string) => i.includes(injectedIncludes)),
        ).toBe(true);
      },
    );
    ...
    it.each([
      ...
      expect(
        result.skipped.some((s: string) => s.includes(skippedIncludes)),
      ).toBe(true);
    });

    it("should update existing theme-color", () => {
      const html =
        '<html><head><meta name="theme-color" content="#000000" /></head><body></body></html>';
      const options: MetaInjectorOptions = { themeColor: "#ffffff" };

      const { html: modifiedHtml, result } = injectMetaTags(html, options);

      expect(modifiedHtml).toContain('content="#ffffff"');
      expect(result.injected.some((i: string) => i.includes("updated"))).toBe(
        true,
      );
    });
    ...
    it("should create head if missing", () => {
      ...
      expect(result.warnings.some((w: string) => w.includes("Created"))).toBe(
        true,
      );
    });
    ...
    describe("Security: XSS prevention", () => {
      ...
      it.each<XssCase>([
        ...
        const match = modifiedHtml.match(
          /navigator\.serviceWorker\.register\(([^)]+)\)/,
        );
  ...
  describe("PWA install handler script injection", () => {
    ...
    it("should inject install script before </body> tag", () => {
      ...
      const scriptIndex = modifiedHtml.indexOf(
        "window.addEventListener('beforeinstallprompt'",
      );
    ...
    it("should inject install script before </html> if no </body> tag", () => {
      ...
      expect(
        result.warnings.some((w: string) => w.includes("(no </body> found)")),
      ).toBe(true);
    });
    ...
    it("should inject install script at end if no </body> or </html> tag", () => {
      ...
      expect(
        result.warnings.some((w: string) =>
          w.includes("(no </body> or </html> found)"),
        ),
      ).toBe(true);
    });
  ...
  describe("Edge cases and error handling", () => {
    ...
    it("should preserve apple-mobile-web-app-capable while also injecting mobile-web-app-capable", () => {
      const html =
      ...
      // IMPORTANT: apple-mobile-web-app-capable must be preserved (never removed)
      expect(modifiedHtml).toContain('name="apple-mobile-web-app-capable"');
      // mobile-web-app-capable should also be injected for Android support
      expect(modifiedHtml).toContain('name="mobile-web-app-capable"');
      // Should not warn about removal - it's preserved
      expect(
        result.warnings.some((w: string) => w.includes("Removed deprecated")),
      ).toBe(false);
    });
    ...
    it("should handle existing mobile-web-app-capable", () => {
      const html =
        '<html><head><meta name="mobile-web-app-capable" content="yes" /></head><body></body></html>';
      const options: MetaInjectorOptions = {
        appleMobileWebAppCapable: true,
      };
    ...
    it("should update existing mobile-web-app-capable with different content", () => {
      const html =
        '<html><head><meta name="mobile-web-app-capable" content="no" /></head><body></body></html>';
      const options: MetaInjectorOptions = {
        appleMobileWebAppCapable: true,
      };
      ...
      expect(
        result.injected.some(
          (i: string) =>
            i.includes("mobile-web-app-capable") && i.includes("updated"),
        ),
      ).toBe(true);
    });
    ...
    it("should handle existing apple-mobile-web-app-status-bar-style", () => {
      const html =
        '<html><head><meta name="apple-mobile-web-app-status-bar-style" content="black" /></head><body></body></html>';
      const options: MetaInjectorOptions = {
        appleMobileWebAppStatusBarStyle: "black",
      };
      ...
      expect(
        result.skipped.some((s: string) =>
          s.includes("apple-mobile-web-app-status-bar-style"),
        ),
      ).toBe(true);
    });
    ...
    it("should update existing apple-mobile-web-app-status-bar-style with different content", () => {
      const html =
        '<html><head><meta name="apple-mobile-web-app-status-bar-style" content="default" /></head><body></body></html>';
      const options: MetaInjectorOptions = {
        appleMobileWebAppStatusBarStyle: "black",
      };
      ...
      expect(
        result.injected.some(
          (i: string) =>
            i.includes("apple-mobile-web-app-status-bar-style") &&
            i.includes("updated"),
        ),
      ).toBe(true);
    });
    ...
    it("should handle existing apple-mobile-web-app-title", () => {
      const html =
        '<html><head><meta name="apple-mobile-web-app-title" content="My App" /></head><body></body></html>';
      const options: MetaInjectorOptions = {
        appleMobileWebAppTitle: "My App",
      };
      ...
      expect(
        result.skipped.some((s: string) =>
          s.includes("apple-mobile-web-app-title"),
        ),
      ).toBe(true);
    });
    ...
    it("should update existing apple-mobile-web-app-title with different content", () => {
      const html =
        '<html><head><meta name="apple-mobile-web-app-title" content="Old App" /></head><body></body></html>';
      const options: MetaInjectorOptions = {
        appleMobileWebAppTitle: "My App",
      };
      ...
      expect(
        result.injected.some(
          (i: string) =>
            i.includes("apple-mobile-web-app-title") && i.includes("updated"),
        ),
      ).toBe(true);
    });
    ...
    it("should handle body creation when html exists but body does not", () => {
      ...
      expect(modifiedHtml).toContain("navigator.serviceWorker.register");
      expect(
        result.warnings.some((w: string) => w.includes("Created <body>")),
      ).toBe(true);
    });
    ...
    it("should handle case where html tag does not exist for body creation", () => {
      ...
      expect(modifiedHtml).toContain("navigator.serviceWorker.register");
      expect(
        result.warnings.some((w: string) => w.includes("No <html> or <body>")),
      ).toBe(true);
    });
    ...
    it("should handle install handler exists but service worker missing", () => {
      const html = baseHtml(
        '<script>window.addEventListener("beforeinstallprompt")</script>',
      );
      const options: MetaInjectorOptions = {
        serviceWorkerPath: "/sw.js",
      };
      ...
      expect(modifiedHtml).toContain("navigator.serviceWorker.register");
      expect(
        result.injected.some((i: string) =>
          i.includes("Service Worker registration"),
        ),
      ).toBe(true);
