...
import { mkdirSync, writeFileSync, unlinkSync, rmdirSync, existsSync, readFileSync } from 'node:fs'
import { join } from 'node:path'
import { tmpdir } from 'node:os'
import { generateServiceWorkerFromConfig } from './service-worker-generator.js'
import type { ServiceWorkerConfig, AdvancedCachingConfig } from './caching-strategy.js'
...
describe('advanced-caching-integration', () => {
  ...
  describe('Service Worker with Advanced Caching', () => {
    ...
    it('should generate cache version with auto versioning', async () => {
      const advancedConfig: AdvancedCachingConfig = {
        routes: [],
        versioning: {
          autoVersion: true,
    ...
    it('should use manual version when provided', async () => {
      const advancedConfig: AdvancedCachingConfig = {
        routes: [],
        versioning: {
    ...
    it('should handle dependency tracking in advanced config', async () => {
      const advancedConfig: AdvancedCachingConfig = {
        routes: [
          {
            pattern: '/app.js',
    ...
    it('should respect ignore patterns in invalidation', async () => {
      writeFileSync(join(testDir, 'public', 'app.js'), 'console.log("app")')
