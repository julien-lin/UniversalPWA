...
const makeOptions = (
  overrides: Partial<ManifestGeneratorOptions> = {},
...
describe("manifest-generator", () => {
  ...
  describe("generateManifest", () => {
    ...
    it("should generate full manifest with all options", () => {
      const manifest = generateManifest(
        makeOptions({
          ...
          icons: [
            makeIcon("/icon-192.png", "192x192"),
            makeIcon("/icon-512.png", "512x512"),
          ],
    ...
    it("should truncate short_name > 12 characters", () => {
      const manifest = generateManifest(
        makeOptions({ shortName: "This is too long" }),
      );
  ...
  describe("manifest id field", () => {
    it("should include id when provided", () => {
      const manifest = generateManifest(
        makeOptions({ id: "my-app-unique-id" }),
      );
      expect(manifest.id).toBe("my-app-unique-id");
    });
    ...
    it("should write id to manifest.json file", () => {
      const options = makeOptions({ id: "test-app-123" });
      const manifestPath = generateAndWriteManifest(options, TEST_DIR);

      const content = readManifest(manifestPath);
      expect(content.id).toBe("test-app-123");
    });

    it("should support scoped id with basePath hash", () => {
      const options = makeOptions({ id: "my-app-a1b2c3d4" });
      const manifest = generateManifest(options);
      expect(manifest.id).toMatch(/^[a-z0-9-]+$/);
    });

    it("should validate manifest schema with id field", () => {
      const manifest = generateManifest(makeOptions({ id: "my-scoped-id" }));
      const validated = ManifestSchema.parse(manifest);
      expect(validated.id).toBe("my-scoped-id");
    });

    it("should prevent collision risk for same app with different basePaths", () => {
      // Scenario: Two PWAs on same domain with different basePaths
      const id1 = "my-app"; // root basePath
