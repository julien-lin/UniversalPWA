...
function generateAdaptiveIconXML(
  ...
  const foregroundRelative = foregroundPath
    .replace(outputDir, "")
    .replace(/^\//, "");
...
export async function generateAdaptiveIcon(
  ...
  const { safeZone = { width: 66, height: 66 } } = config;
  ...
  if (background.type === "color") {
    ...
    const colorValue = background.value.startsWith("#")
      ? background.value
      : `#${background.value}`;
    bgImage = sharp({
      create: {
  ...
  const xmlPath = generateAdaptiveIconXML(
    outputDir,
    foregroundPath,
    backgroundPath,
  );
...
export async function generateAdaptiveIconFromSource(
  ...
  return generateAdaptiveIcon(
    source,
    adaptiveConfig.background!,
    outputDir,
    adaptiveConfig,
  );
}
