...
describe.sequential("service-worker-generation-comprehensive", () => {
  ...
  describe("Unit Tests - All Templates", () => {
    ...
    it.each(templateTypes)(
      "should generate service worker with %s template",
      async (templateType) => {
        try {
          ...
          if (
            ...
            error.message.includes("Unknown service worker template type")
          ) {
            return;
          }
          throw error;
    ...
    it.each(templateTypes)(
      "should generate valid Workbox code for %s template",
      async (templateType) => {
        try {
          const result = await generateServiceWorker({
            projectPath: testDir,
  ...
  describe("Integration Tests - Real Project Scenarios", () => {
    it("should generate service worker for static site with multiple assets", async () => {
      ...
      writeFileSync(
        join(testDir, "public", "assets", "main.js"),
        'console.log("main")',
      );
      writeFileSync(
        join(testDir, "public", "assets", "main.css"),
        "body { color: red; }",
      );
      writeFileSync(
        join(testDir, "public", "images", "logo.png"),
        Buffer.from("logo"),
      );
      writeFileSync(
        join(testDir, "public", "about.html"),
        "<html><body>About</body></html>",
      );
      writeFileSync(
        join(testDir, "public", "contact.html"),
    ...
    it("should generate service worker for SSR with offline support", async () => {
      writeFileSync(
        join(testDir, "public", "offline.html"),
  ...
  describe("Performance Benchmarks", () => {
    ...
    it("should handle large number of files efficiently", async () => {
      ...
      for (let i = 0; i < 50; i++) {
        writeFileSync(
          join(testDir, "public", `file-${i}.js`),
          `console.log("file ${i}")`,
        );
        writeFileSync(
          join(testDir, "public", `file-${i}.css`),
          `/* file ${i} */`,
        );
      }
  ...
  describe("Edge Cases and Error Handling", () => {
    ...
    it("should handle special characters in file paths", async () => {
      writeFileSync(
        join(testDir, "public", "file with spaces.html"),
        "<html></html>",
      );
      writeFileSync(
        join(testDir, "public", "file-with-dashes.html"),
        "<html></html>",
      );
      writeFileSync(
        join(testDir, "public", "file_with_underscores.html"),
        "<html></html>",
      );
    ...
    it("should handle nested directory structures", async () => {
      mkdirSync(join(testDir, "public", "nested", "deep", "path"), {
        recursive: true,
      });
      writeFileSync(
        join(testDir, "public", "nested", "deep", "path", "file.js"),
        'console.log("deep")',
      );
  ...
  describe("Template-Specific Validation", () => {
    ...
    it("should generate framework-specific templates correctly", async () => {
      ...
      const frameworks: Array<"laravel-spa" | "symfony-spa" | "flask-spa"> = [
        "laravel-spa",
        "symfony-spa",
        "flask-spa",
      ];
