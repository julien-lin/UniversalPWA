...
} from "../validator/icon-validator.js";
...
import { generateAdaptiveIcon } from "./adaptive-icon-generator.js";
...
export async function generateIconsAdvanced(
  ...
  if (!bestSource || !existsSync(bestSource.path)) {
    throw new Error(
      `No valid icon source found. Checked ${sources.length} source(s)`,
    );
  }
  ...
  if (parallel) {
    const iconResults = await processInParallel(
      ...
      async (size) => {
        ...
        const optimalQuality = optimize
          ? getOptimalQuality(size, quality)
          : quality;
        ...
        try {
          ...
          return {
            ...
            icon: {
              ...
              purpose:
                size.width >= 192 && size.width <= 512 ? "any" : undefined,
            },
          };
        } catch (err: unknown) {
  ...
  try {
    ...
    logger.warn(
      { module: "icon-generator" },
      `Failed to generate apple-touch-icon: ${message}`,
    );
  }
  ...
  if (config.adaptiveIcons?.enabled) {
    try {
      ...
      if (!foregroundSource) {
        logger.warn(
          { module: "icon-generator" },
          "Adaptive icons enabled but no foreground source available",
        );
      } else {
        ...
        logger.info(
          {
            module: "icon-generator",
            files: adaptiveResult.generatedFiles.length,
          },
      ...
      logger.warn(
        { module: "icon-generator" },
        `Failed to generate adaptive icons: ${message}`,
      );
    }
  }
  ...
  if (config.splashScreens?.enabled) {
    try {
      ...
      if (!splashSource) {
        logger.warn(
          { module: "icon-generator" },
          "Splash screens enabled but no source available",
        );
      } else {
        const splashResult = await generateSplashScreens(
          splashSource,
          splashConfig,
          splashOutputDir,
        );
        ...
        for (const androidPath of splashResult.android) {
          const relativePath = androidPath
            .replace(outputDir, "")
            .replace(/^\//, "");
        ...
        logger.info(
          {
            module: "icon-generator",
            ios: splashResult.ios.length,
            android: splashResult.android.length,
          },
      ...
      logger.warn(
        { module: "icon-generator" },
        `Failed to generate splash screens: ${message}`,
      );
    }
  }
 ...
 */
export async function generateIconsOnly(
  options: IconGeneratorOptions,
): Promise<{
  icons: ManifestIcon[];
  generatedFiles: string[];
 ...
 */
export async function generateSplashScreensOnly(
  options: IconGeneratorOptions,
): Promise<{
  splashScreens: ManifestSplashScreen[];
  generatedFiles: string[];
