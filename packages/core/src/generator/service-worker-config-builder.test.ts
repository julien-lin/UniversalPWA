...
const mockBackendIntegration = (
  overrides?: Partial<BackendIntegration>,
): BackendIntegration =>
  ({
    id: "test-backend",
    name: "Test Backend",
...
describe("ServiceWorkerConfigBuilder", () => {
  describe("fromBackendIntegration", () => {
    it("should create config from backend integration", () => {
      ...
      const config = ServiceWorkerConfigBuilder.fromBackendIntegration(
        backend,
        "sw.js",
      );
  ...
  describe("validate", () => {
    ...
    it("should report missing destination", () => {
      ...
      expect(validation.errors.some((e) => e.includes("destination"))).toBe(
        true,
      );
    });
    ...
    it("should report missing route arrays", () => {
      ...
      expect(validation.isValid).toBe(false);
      expect(validation.errors.some((e) => e.includes("staticRoutes"))).toBe(
        true,
      );
    });
    ...
    it("should warn about empty routes", () => {
      ...
      expect(validation.warnings.some((w) => w.includes("No routes"))).toBe(
        true,
      );
    });
    ...
    it("should warn about duplicate patterns", () => {
      ...
      expect(validation.warnings.some((w) => w.includes("duplicate"))).toBe(
        true,
      );
    });
  ...
  describe("createDefault", () => {
    ...
    it("should normalize basePath without trailing slash", () => {
      const config = ServiceWorkerConfigBuilder.createDefault(
        "sw.js",
        "/api/v1/pwa",
      );
