...
import {
  ...
  generateAdaptiveIconFromSource,
} from "./adaptive-icon-generator.js";
import type {
  IconSource,
  ColorSource,
  AdaptiveIconConfig,
...
describe("adaptive-icon-generator", () => {
  ...
  describe("generateAdaptiveIcon", () => {
    it("should generate adaptive icon with color background", async () => {
      ...
      const result = await generateAdaptiveIcon(
        foreground,
        background,
        outputDir,
        config,
    ...
    it("should generate density-specific icons", async () => {
      ...
      for (const density of densities) {
        const densityPath = result.densities.find((p) =>
          p.includes(`mipmap-${density}`),
        );
  ...
  describe("generateAdaptiveIconFromSource", () => {
    it("should generate adaptive icon from single source with color background", async () => {
      ...
      const result = await generateAdaptiveIconFromSource(
        source,
        "#ff0000",
        outputDir,
      );
