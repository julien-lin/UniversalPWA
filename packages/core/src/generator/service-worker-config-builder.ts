...
export class ServiceWorkerConfigBuilder {
  ...
  static fromBackendIntegration(
    ...
    if (options?.deduplicateRoutes !== false) {
      config.staticRoutes = RoutePatternResolver.deduplicate(
        config.staticRoutes,
      );
      ...
      if (config.customRoutes) {
        config.customRoutes = RoutePatternResolver.deduplicate(
          config.customRoutes,
        );
      }
    }
    ...
    if (options?.validate !== false) {
      ...
      if (!validation.isValid) {
        logger.warn(
          {
            module: "service-worker-config-builder",
            errors: validation.errors,
          },
          "ServiceWorkerConfig validation warnings",
        );
      }
    }
  ...
  static merge(...configs: ServiceWorkerConfig[]): ServiceWorkerConfig {
    ...
    merged.customRoutes = RoutePatternResolver.deduplicate(
      merged.customRoutes || [],
    );
  ...
  static validate(config: ServiceWorkerConfig): ValidationResult {
    ...
    if (
      ...
      config.apiRoutes.length === 0 &&
      config.imageRoutes.length === 0
    ) {
      warnings.push(
        "No routes configured (staticRoutes, apiRoutes, imageRoutes are all empty)",
      );
    }
    ...
    const duplicates = allPatterns.filter(
      (p, i) => allPatterns.indexOf(p) !== i,
    );
    ...
    if (config.offline) {
      if (
        ...
        typeof config.offline.fallbackPage !== "string"
      ) {
        errors.push("offline.fallbackPage must be a string");
      }
      if (
        config.offline.fallbackImage &&
        typeof config.offline.fallbackImage !== "string"
      ) {
  ...
  build(): ServiceWorkerConfig {
    ...
    return {
      ...
      customRoutes: this.config.customRoutes
        ? RoutePatternResolver.deduplicate(this.config.customRoutes)
        : undefined,
    };
  }
