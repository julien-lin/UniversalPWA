...
export function verifyCacheSignature(
  ...
  try {
    ...
    const expectedSignature = generateCacheSignature(manifestContent, secret, config)
...
export function extractSignatureFromManifest(manifestContent: string): {
  ...
  const signatureMatch = manifestContent.match(/^\/\/\s*HMAC:\s*([a-f0-9]+|[A-Za-z0-9+/=]+)\s*\n/)
  if (signatureMatch) {
    return {
 ...
 */
export function prependSignatureToManifest(manifestContent: string, signature: string): string {
 ...
 */
export function formatCacheIntegrityError(result: CacheIntegrityResult): string {
  if (result.isValid) {
