...
describe("sharp-limits", () => {
  ...
  describe("ICON_SHARP_LIMITS", () => {
    it("should have smaller dimensions than default", () => {
      expect(ICON_SHARP_LIMITS.maxWidth).toBeLessThan(
        DEFAULT_SHARP_LIMITS.maxWidth,
      );
      expect(ICON_SHARP_LIMITS.maxHeight).toBeLessThan(
        DEFAULT_SHARP_LIMITS.maxHeight,
      );
    });
    ...
    it("should allow higher concurrency for icon batch processing", () => {
      expect(ICON_SHARP_LIMITS.maxConcurrency).toBeGreaterThan(
        DEFAULT_SHARP_LIMITS.maxConcurrency,
      );
    });

    it("should have smaller max input size per icon", () => {
      expect(ICON_SHARP_LIMITS.maxInputSize).toBeLessThan(
        DEFAULT_SHARP_LIMITS.maxInputSize,
      );
    });
  });
  ...
  describe("validateImageFileSize", () => {
    it("should accept file size within limits", () => {
      const result = validateImageFileSize(
        5 * 1024 * 1024,
        DEFAULT_SHARP_LIMITS,
      );
      expect(result.isValid).toBe(true);
  ...
  describe("Security scenarios", () => {
    ...
    it("should enforce icon limits for batch processing", () => {
      ...
      const asGeneral = validateImageDimensions(
        1024,
        1024,
        DEFAULT_SHARP_LIMITS,
      );
