...
describe('timeout', () => {
  describe('DEFAULT_TIMEOUTS', () => {
    ...
    it('should have reasonable timeout progression', () => {
      expect(DEFAULT_TIMEOUTS.fileOperation).toBeLessThan(DEFAULT_TIMEOUTS.imageProcessing)
      expect(DEFAULT_TIMEOUTS.imageProcessing).toBeLessThanOrEqual(DEFAULT_TIMEOUTS.initCommand)
    })
  })
  ...
  describe('parallelWithTimeout', () => {
    it('should execute all operations in parallel', async () => {
      ...
      expect(results.map((r) => ('result' in r ? r.result : null))).toContain(1)
      expect(results.map((r) => ('result' in r ? r.result : null))).toContain(2)
      expect(results.map((r) => ('result' in r ? r.result : null))).toContain(3)
    })
  ...
  describe('sequentialWithTimeout', () => {
    it('should execute operations sequentially', async () => {
      const order: string[] = []
      const operations = [
        { name: 'op1', fn: async () => { order.push('op1'); return 1 } },
        { name: 'op2', fn: async () => { order.push('op2'); return 2 } },
        { name: 'op3', fn: async () => { order.push('op3'); return 3 } },
      ]
  ...
  describe('createBatchProcessor', () => {
    it('should process items in batches', async () => {
      ...
      const results = await processor(items, (x: number) => Promise.resolve(x * 2))
    ...
    it('should handle batch timeout', async () => {
      ...
      const slowProcessor = (x: number) => new Promise<number>((resolve) => {
