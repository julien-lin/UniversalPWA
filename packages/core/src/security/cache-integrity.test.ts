...
describe('cache-integrity', () => {
  ...
  describe('generateCacheSignature', () => {
    ...
    it('should generate different signatures for different secrets', () => {
      const sig1 = generateCacheSignature(testManifest, testSecret)
      const sig2 = generateCacheSignature(testManifest, 'different-secret-key-thats-long-enough')
      expect(sig1).not.toBe(sig2)
    })
    ...
    it('should support base64 encoding', () => {
      const config = { ...DEFAULT_CACHE_INTEGRITY_CONFIG, encoding: 'base64' as const }
    ...
    it('should reject short secrets', () => {
      expect(() => generateCacheSignature(testManifest, 'short')).toThrow('must be at least 16 characters')
    })

    it('should support SHA512', () => {
      const config = { ...DEFAULT_CACHE_INTEGRITY_CONFIG, algorithm: 'sha512' as const }
      const sig = generateCacheSignature(testManifest, testSecret, config)
  ...
  describe('verifyCacheSignature', () => {
    ...
    it('should reject invalid signatures', () => {
      ...
      const result = verifyCacheSignature(testManifest, invalidSignature, testSecret)
    ...
    it('should reject modified manifest', () => {
      ...
      const result = verifyCacheSignature(modifiedManifest, signature, testSecret)
      expect(result.isValid).toBe(false)
    })

    it('should reject wrong secret', () => {
      const signature = generateCacheSignature(testManifest, testSecret)
      const result = verifyCacheSignature(testManifest, signature, 'wrong-secret-key-thats-long-enough')
      expect(result.isValid).toBe(false)
    })
  ...
  describe('createValidationMetadata', () => {
    it('should create metadata with timestamp', () => {
      const signature = generateCacheSignature(testManifest, testSecret)
      const result = verifyCacheSignature(testManifest, signature, testSecret)
      const metadata = createValidationMetadata(testManifest, result, DEFAULT_CACHE_INTEGRITY_CONFIG)
  ...
  describe('CacheIntegrityError', () => {
    it('should create error with manifest path', () => {
      const error = new CacheIntegrityError('/path/to/manifest.json', 'Test error')
  ...
  describe('Security scenarios', () => {
    ...
    it('should prevent cache poisoning attacks', () => {
      ...
      const result = verifyCacheSignature(maliciousManifest, signature, testSecret)
    ...
    it('should use timing-safe comparison', () => {
      ...
      const wrong1 = 'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa'
      const wrong2 = 'ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff'
  ...
  describe('Integration scenarios', () => {
    ...
    it('should handle different algorithms in round-trip', () => {
      const config = { ...DEFAULT_CACHE_INTEGRITY_CONFIG, algorithm: 'sha512' as const }
      const signature = generateCacheSignature(testManifest, testSecret, config)
      const result = verifyCacheSignature(testManifest, signature, testSecret, config)
