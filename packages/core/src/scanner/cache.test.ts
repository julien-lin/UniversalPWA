...
const makeResult = (
  projectPath: string,
  overrides: DeepPartial<ScannerResult> = {},
): ScannerResult => ({
  framework: {
    framework: null,
    confidence: "low",
    ...
    configuration: {
      ...
      stateManagement: [],
      buildTool: null,
      ...((overrides.framework?.configuration as Partial<
        ScannerResult["framework"]["configuration"]
      >) ?? {}),
    },
...
describe("cache", () => {
  ...
  describe("isCacheValid", () => {
    ...
    it("should return true for valid cache entry", () => {
      ...
      const cache = withUpdatedCache(TEST_DIR, result, {
        version: "1.0.0",
        entries: {},
      });
      saveCache(cache, cacheFile);
  ...
  describe("getCachedResult", () => {
    ...
    it("should return cached result", () => {
      ...
      const cache = updateCache(TEST_DIR, result, {
        version: "1.0.0",
        entries: {},
      });
  ...
  describe("updateCache", () => {
    ...
    it("should update existing cache entry", () => {
      ...
      const result2 = makeResult(TEST_DIR, {
