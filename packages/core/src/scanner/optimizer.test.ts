...
const expectOptimizationResultOk = (
  result: Awaited<ReturnType<typeof optimizeProject>>,
) => {
...
describe("optimizer", () => {
  ...
  describe("detectApiType", () => {
    ...
    it("should handle missing JavaScript files gracefully", () => {
      const assets = makeAssets({
        javascript: [join(TEST_DIR, "non-existent.js")],
      });
      const apiType = detectApiType(TEST_DIR, assets);
      expect(apiType).toBeDefined();
    });
  ...
  describe("generateAdaptiveCacheStrategies", () => {
    it.each([
      ...
      "should generate strategies for $name",
      ({ apiType, assets, config, expect: exp }) => {
        const strategies = generateAdaptiveCacheStrategies(
          apiType,
          assets,
          config,
        ) as Array<{
  ...
  describe("detectUnoptimizedImages", () => {
    ...
    it("should suggest optimization for large PNG", () => {
      ...
      expect(
        suggestions.some(
          (s) => s.suggestion.includes("PNG") || s.suggestion.includes("WebP"),
        ),
      ).toBe(true);
    });
    ...
    it("should handle non-existent images gracefully", () => {
      const assets = makeAssets({
        images: [join(TEST_DIR, "non-existent.png")],
      });
      const suggestions = detectUnoptimizedImages(assets);
  ...
  describe("generateOptimalShortName", () => {
    it.each<[name: string, input: string, assertFn: (r: string) => void]>([
  ...
  describe("optimizeImage", () => {
    it("should optimize image and convert to WebP", async () => {
      ...
      await createSharpImage(testImagePath, 1000, 1000, {
        r: 255,
        g: 0,
        b: 0,
        alpha: 1,
    ...
    it("should resize image if maxWidth specified", async () => {
      ...
      await createSharpImage(testImagePath, 2000, 1500, {
        r: 128,
        g: 128,
        b: 128,
        alpha: 1,
    ...
    it("should handle optimization errors gracefully", async () => {
      const testImagePath = join(TEST_DIR, "test-image.png");
      await createSharpImage(testImagePath, 100, 100, {
        r: 255,
        g: 0,
        b: 0,
        alpha: 1,
    ...
    it("should keep WebP as WebP if already WebP", async () => {
      ...
      await createSharpImage(join(TEST_DIR, "temp.png"), 500, 500, {
        r: 255,
        g: 255,
        b: 0,
        alpha: 1,
  ...
  describe("generateResponsiveImageSizes", () => {
    it("should generate responsive image sizes", async () => {
      ...
      const result = await generateResponsiveImageSizes(
        testImagePath,
        outputDir,
        [320, 640, 1024],
      );
    ...
    it("should not upscale images", async () => {
      ...
      await createSharpImage(testImagePath, 200, 200, {
        r: 0,
        g: 0,
        b: 255,
        alpha: 1,
  ...
  describe("optimizeProjectImages", () => {
    ...
    it("should optimize first 10 images if no high priority", async () => {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        await createSharpImage(imgPath, 500, 500, {
          r: 255,
          g: 0,
          b: 0,
          alpha: 1,
    ...
    it("should only include results with savings > 0", async () => {
      ...
      await createSharpImage(imagePath, 100, 100, {
        r: 255,
        g: 255,
        b: 255,
        alpha: 1,
  ...
  describe("generateOptimalShortName", () => {
    ...
    it("should handle very long single word", () => {
      const result = generateOptimalShortName(
