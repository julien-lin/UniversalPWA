...
describe("Path Security Validator - P3.2", () => {
  ...
  describe("checkPathTraversal", () => {
    ...
    it("should reject paths with .. traversal", () => {
      ...
      expect(result.violations.some((v: string) => v.includes(".."))).toBe(
        true,
      );
    });
    ...
    it("should reject paths with null bytes", () => {
      ...
      expect(result.safe).toBe(false);
      expect(result.hasTraversal).toBe(true);
      expect(result.violations.some((v: string) => v.includes("null"))).toBe(
        true,
      );
    });
  ...
  describe("validatePath", () => {
    ...
    it("should check path length", () => {
      ...
      expect(result.valid).toBe(false);
      expect(result.errors.some((e: string) => e.includes("max length"))).toBe(
        true,
      );
    });
