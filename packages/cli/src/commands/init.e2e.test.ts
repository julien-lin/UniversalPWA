...
const createDjangoProject = (): string => {
  const root = join(
    tmpdir(),
    `django-e2e-${Date.now()}-${Math.random().toString(36).slice(2)}`,
  );
  ...
  writeFileSync(join(root, "requirements.txt"), "Django==4.2.0\n");
  writeFileSync(join(root, "myapp", "__init__.py"), "");
  writeFileSync(
...
const createReactViteProject = (): string => {
  const root = join(
    tmpdir(),
    `react-vite-e2e-${Date.now()}-${Math.random().toString(36).slice(2)}`,
  );
  mkdirSync(root, { recursive: true });
...
const createLaravelProject = (): string => {
  const root = join(
    tmpdir(),
    `laravel-e2e-${Date.now()}-${Math.random().toString(36).slice(2)}`,
  );
  mkdirSync(root, { recursive: true });
  ...
  writeFileSync(join(root, "public", "index.php"), '<?php echo "Laravel";');
...
describe("CLI E2E Tests", () => {
  ...
  describe("Laravel Project Workflow", () => {
    ...
    it("should have Laravel directories and files", () => {
      ...
      expect(
        existsSync(
          join(projectPath, "resources", "views", "welcome.blade.php"),
        ),
      ).toBe(true);
    });
  ...
  describe("Cross-Framework Features", () => {
    ...
    it("should have HTML entry points", () => {
      // Django
      expect(
        existsSync(join(djangoPath, "myapp", "templates", "index.html")),
      ).toBe(true);
      // React Vite
      ...
      expect(
        existsSync(
          join(laravelPath, "resources", "views", "welcome.blade.php"),
        ),
      ).toBe(true);
    });
  });
