...
import { findConfigFile, loadConfig, type ConfigLoadResult } from '@julien-lin/universal-pwa-core'
 ...
 */
export async function loadProjectConfig(projectPath: string): Promise<MergedConfig> {
  const resolvedPath = resolve(projectPath)

  try {
    const configFilePath = await findConfigFile(resolvedPath)
    ...
    console.warn(`⚠️  Failed to load configuration file: ${error instanceof Error ? error.message : String(error)}`)
    return {
      config: null,
      filePath: null,
      format: null,
