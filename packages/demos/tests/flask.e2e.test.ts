...
test.describe('E2E - Flask (Python)', () => {
  const BASE_URL = 'http://localhost:3006'
  const hasFixture = existsSync(join(process.cwd(), 'fixtures/flask'))

  test.skip(!hasFixture, 'Fixture flask absente')
  ...
  test('should load the Flask app', async ({ page }) => {
    // Vérifier que l'app Flask se charge
    await expect(page.locator('h1')).toContainText(/Flask Test App/i)
    await expect(page).toHaveTitle(/Flask Test App/i)
  })
  ...
  test('should have CSRF token meta tag (Flask specific)', async ({ page }) => {
    const csrfToken = page.locator('meta[name="csrf-token"]')
    await expect(csrfToken).toHaveAttribute('content', 'flask-fixture-token')
  })
  ...
  test('should have service worker file accessible', async ({ page, request }) => {
    ...
    // Vérifier les routes Flask spécifiques
